...
def parse_sam_file (samfile):
    ...
    samfile = pandas.read_table(samfile, sep='\t', comment='@', names=[
        ...
        'X0', 'X1', 'XM', 'XO', 'XG', 'MDZ'  # TODO: fix this, different SAM files might have different columns. Look at pysam to easily parse this
    ])
...
def check_for_duplicate_reads(samfile):
    # TODO: what does 'duplicate reads' mean in this context? same position? same flag + position? same index?
    # TODO: rename 'samfile' as it isn't a sam file
    # TODO: clarify whether or not you keep one or zero copies of duplicated reads (better to keep one?)
    """
    ...
    not contain any duplicate reads. Duplicate reads have the same start, strand, same cigar string, same sequence
